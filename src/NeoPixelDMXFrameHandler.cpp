...
NeoPixelDMXFrameHandler::NeoPixelDMXFrameHandler() : currentBrightness(DEFAULT_BRIGHTNESS)
{
}

// ----------------- IDMXFrameHandler Implementation -----------------
void NeoPixelDMXFrameHandler::handleFrame(uint16_t universeIn,
                                          uint16_t length,
                                          uint8_t* data)
{
  ...
  if (colorMode == COLOR_MODE_SINGLE)
  {
    ...
    if (USE_DIM_CHANNEL)
    {
      ...
      color
          = RgbColor(data[startAddress], data[startAddress + 1], data[startAddress + 2]);
    ...
    else
    {
      ...
      color
          = RgbColor(data[startAddress - 1], data[startAddress], data[startAddress + 1]);
      if (color != currentColor)
      {
        currentColor = color;
        needsUpdate  = true;
  ...
  {  // COLOR_MODE_MULTIPLE
    for (int i = 0; i < numLeds; i++)
    {
      ...
      if (USE_DIM_CHANNEL)
      {
        ...
        RgbColor color(data[offset + startAddress],
                       data[offset + startAddress + 1],
                       data[offset + startAddress + 2]);
        scaledColor = color.Dim(brightness);
      }
      else
      {
        RgbColor color(data[offset + startAddress - 1],
                       data[offset + startAddress],
                       data[offset + startAddress + 1]);
...
int NeoPixelDMXFrameHandler::getFrameRate() const { return currentFps; }
